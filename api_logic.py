...
class APILogic:
    ...
    def get_assets(self):
        assets = self.api.get_asset(
            columns=['ASSET_DATABASE_ID', 'IS_PORTFOLIO', 'TYPE'])

        assets_list = []
        if (assets is not None):
            ids = list(
                map(lambda x: int(x['ASSET_DATABASE_ID']['value']), assets))
                ...
                if item['IS_PORTFOLIO']['value'] == 'false':
                    ...
                    if asset_ratio_json is not None:
                        asset_ratio = AssetRatio(
                            ...
                            float(asset_ratio_json[self.api.ID_SHARPE]['value'].replace(
                                ',', '.')) if check_ratio(asset_ratio_json[self.api.ID_SHARPE]) else None,
                            float(asset_ratio_json[self.api.ID_VOL]['value'].replace(
                                ',', '.')) if check_ratio(asset_ratio_json[self.api.ID_VOL]) else None,
                            float(asset_ratio_json[self.api.ID_RETURN]['value'].replace(
                                ',', '.')) if check_ratio(asset_ratio_json[self.api.ID_RETURN]) else None,
                        )
    ...
    def get_assets_nav(self, assets):
        for item in assets:
            asset_quote = self.api.get_asset_quote(
                item.id, startDate=self.api.PERIOD_START_DATE, endDate=self.api.PERIOD_START_DATE)
            if asset_quote is not None and len(asset_quote) > 0:
                item.nav = float(
                    asset_quote[0]['nav']['value'].replace(',', '.'))

    def get_user_ptf(self):
        json = self.api.get_ptf(self.api.ID_PTF_USER)
        if json is not None:
            assets = []
            if json['values'] is not None and json['values'][self.api.PERIOD_START_DATE] is not None:
                for item in json['values'][self.api.PERIOD_START_DATE]:
                    assets.append(Asset(
                        str(item['asset']['asset']),
                        is_portfolio='false',
                        quantity=item['asset']['quantity']
                    ))
            return Portfolio(
                self.api.ID_PTF_USER,
                json['label'],
                sharpe=None,
                assets=assets
            )
